...
class PaintApp:
    ...
    def draw_stroke_on_pil(self, draw, obj):
        ...
        elif brush in ("spray", "airbrush", "crayon", "watercolor", "calligraphy", "neon"):
            for i in range(len(points) - 1):
                x1, y1 = points[i]
                x2, y2 = points[i + 1]
                self.apply_brush_effect_to_pil(draw, brush, x1, y1, x2, y2, width, color)
    ...
    def start_draw(self, event):
        ...
        # Check handles first so rotate works even when handle sits outside object bounds
    ...
    def apply_brush_effect_to_pil(self, draw, brush, x1, y1, x2, y2, width, color):
        if brush == "spray":
            for _ in range(20):
                px = x2 + random.randint(-10, 10)
                py = y2 + random.randint(-10, 10)
                draw.point((px, py), fill=color)

        elif brush == "airbrush":
            for _ in range(35):
                px = x2 + random.randint(-12, 12)
                py = y2 + random.randint(-12, 12)
                draw.point((px, py), fill=color)

        elif brush == "calligraphy":
            draw.line([x1, y1, x2, y2], fill=color, width=max(2, width + 2))
            draw.line([x1 + 2, y1, x2 + 2, y2], fill=color, width=max(1, width // 2))

        elif brush == "crayon":
            for _ in range(10):
                jitter = random.randint(-2, 2)
                draw.line([x1 + jitter, y1 + jitter, x2 + jitter, y2 + jitter],
                          fill=color, width=max(1, width // 2))

        elif brush == "watercolor":
            for _ in range(6):
                jitter = random.randint(-4, 4)
                draw.line([x1 + jitter, y1, x2 + jitter, y2],
                          fill=color, width=max(1, width))

        elif brush == "neon":
            draw.line([x1, y1, x2, y2], fill=color, width=width + 6)
            draw.line([x1, y1, x2, y2], fill="white", width=max(1, width // 2))
